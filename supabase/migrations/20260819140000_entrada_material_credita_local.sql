-- Entrada de material num SATELITE creditava o saldo GLOBAL, nao o do local.
--
-- A funcao tratava todo item_type='warehouse' igual: somava em
-- warehouse_items.current_stock. Isso serve para o Almoxarifado, cujo saldo e
-- global, mas nao para a Satelite Terreo, que tem saldo proprio em item_stocks
-- e e o que a tela dela exibe.
--
-- Resultado pratico: em 19/08/2026 a farmacia lancou 4.525 unidades na Satelite
-- Terreo; os LOTES foram gravados certos no local, mas o saldo foi para o total
-- global e a tela do satelite continuou mostrando o numero antigo — dai o relato
-- de "as quantidades estao todas diferentes do que digitei".
--
-- Correcao: separa o destino. ALMOX segue no saldo global (inalterado);
-- qualquer outro local de material credita item_stocks daquele local.
-- Medicamento (pharmacy) nao foi tocado.
