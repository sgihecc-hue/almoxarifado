/*
  ...
  2. Security
    - Enable RLS on `consumption_entries` table
    - Add policy for authenticated users to read consumption entries
    - Add policy for administrators to insert, update, and delete consumption entries
*/
...
CREATE INDEX IF NOT EXISTS consumption_entries_item_id_idx ON consumption_entries(item_id);
CREATE INDEX IF NOT EXISTS consumption_entries_department_id_idx ON consumption_entries(department_id);
CREATE INDEX IF NOT EXISTS consumption_entries_date_idx ON consumption_entries(date);
CREATE INDEX IF NOT EXISTS consumption_entries_created_by_idx ON consumption_entries(created_by);
...
CREATE TRIGGER audit_consumption_entries_changes
  AFTER INSERT OR UPDATE OR DELETE ON consumption_entries
  FOR EACH ROW EXECUTE FUNCTION audit_log_changes();

-- Create function to update item consumption history based on consumption entries
CREATE OR REPLACE FUNCTION update_item_consumption_from_entries()
RETURNS TRIGGER AS $$
DECLARE
  v_year_month TEXT;
  v_item_id UUID;
...
CREATE TRIGGER update_consumption_history_from_entries
  AFTER INSERT ON consumption_entries
  FOR EACH ROW
  EXECUTE FUNCTION update_item_consumption_from_entries();