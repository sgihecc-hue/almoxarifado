-- Data da RM (Requisicao de Material) na dispensacao por requisicao.
-- Campo OPCIONAL: fica nulo quando o operador nao informa, e nao existe em
-- dispensacoes antigas.
alter table public.pharmacy_dispensations
  add column if not exists rm_date date;

comment on column public.pharmacy_dispensations.rm_date is
  'Data da Requisicao de Material (RM). Opcional, usado no tipo = requisicao.';

-- A RPC ganha p_rm_date. Derruba a assinatura antiga antes de recriar pra nao
-- deixar duas versoes sobrepostas (chamada por parametro nomeado ficaria
-- ambigua). Todo o resto do comportamento e identico a
-- 20260817150000_dispensacao_sem_aprovacao_farmaceutico.sql.
drop function if exists public.criar_dispensacao(text, text, text, text, date, jsonb, uuid, uuid, uuid, text, text, text, boolean, text, text);

CREATE OR REPLACE FUNCTION public.criar_dispensacao(p_patient_name text, p_medical_record_number text, p_prescribing_doctor text, p_prescription_number text, p_prescription_date date, p_items jsonb, p_patient_id uuid DEFAULT NULL::uuid, p_admission_id uuid DEFAULT NULL::uuid, p_prescriber_id uuid DEFAULT NULL::uuid, p_patient_bed_room text DEFAULT NULL::text, p_sector text DEFAULT NULL::text, p_notes text DEFAULT NULL::text, p_mav_confirmado boolean DEFAULT false, p_tipo text DEFAULT 'prescricao'::text, p_source_location_code text DEFAULT 'CAF'::text, p_rm_date date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
