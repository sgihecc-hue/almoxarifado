...
export default function PharmacyTVDashboard() {
  ...
  async function loadRequests() {
    try {
      ...
      // Filter only pharmacy requests that are pending, approved, or in processing
      const pharmacyRequests = data.filter(r => 
        r.type === 'pharmacy' && 
  ...
  const pendingManagementCount = allRequests.filter(r => 
    r.type === 'pharmacy' && (r.status === 'pending' || r.status === 'approved')
  ).length
                ...
                >
                  <div className="grid grid-cols-12 gap-4 items-center">
                    ...
                    <div className="col-span-1 text-center">
                      <span className="bg-blue-900 text-blue-200 px-2 py-1 rounded-full text-xs font-medium">
          ...
          {requests.filter(r => r.status === 'pending').length > 0 ? (
            <div className="grid grid-cols-2 gap-3">
              {requests.filter(r => r.status === 'pending').slice(0, 4).map(request => (
                <div key={request.id} className="bg-yellow-900/20 p-2 rounded-lg border border-yellow-800/50">
